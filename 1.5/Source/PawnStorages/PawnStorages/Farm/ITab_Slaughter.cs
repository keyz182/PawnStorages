...
namespace PawnStorages.Farm
{
    public class ITab_Slaughter : ITab
    {
        ...
        public List<AutoSlaughterConfig> AutoSlaughterSettings => compFarmBreeder.AutoSlaughterSettings;

        public override void OnOpen()
        {
            compFarmBreeder.AutoSlaughterSettings = compFarmBreeder.AutoSlaughterSettings.OrderByDescending(c => compFarmStorage.CountForType(c.animal).total).ThenBy(c => c.animal.label).ToList();
        }
        ...
        public void DoMaxColumn(WidgetRow row, ref int val, ref string buffer, int current)
        {
            ...
            if (val == -1)
            {
                ...
                row.Gap(width);
                if (row.ButtonIconWithBG(TexButton.Infinity, 48f, (string) "AutoSlaughterTooltipSetLimit".Translate()))
                {
        ...
        public override void FillTab()
        {
            ...
            int row = 0;
            foreach (AutoSlaughterConfig config in AutoSlaughterSettings)
            {
                Rect rowRect = listingStandard.GetRect(24f);
                if (rowRect.Overlaps(other))
                    DrawLine(rowRect, config, row++);
                listingStandard.Gap(6f);
            }
            listingStandard.End();
